...
fgsc2 <- rbind(s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14, s15, s16, s19, s20, s21, s22, s23, s24, s25, s26, s27, s29, s30, s31, s32, s33)
...
datatable(sd, 
          ...
          escape = TRUE,
          rownames = FALSE, 
          colnames = c('Code', 'Host', 'Country', 'Year', 'Lat', 'Long', 'FGSC', 'Tri genotype', 'Pub1'), 
...
library(htmltools)
pal <- colorFactor("viridis", domain = c("Fgra", "Fmer", "Fasi", "Fcor", "Faus", "Flou", "Fboo", "Fger", "Fbra", "Fboo x Fgra", "Faca", "Fuss", "Fvor"))
leaflet(data = sd) %>%  
  addTiles(urlTemplate = "https://mts1.google.com/vt/lyrs=r&hl=en&src=app&x={x}&y={y}&z={z}&s=G", attribution = 'Google') %>% 
    addCircleMarkers(group = "FGSC", radius = 7, fillOpacity = 0.8,  weight=1, color = NA, fillColor = ~pal(FGSC), label = ~FGSC) %>%
