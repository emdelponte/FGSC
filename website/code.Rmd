...
library(gsheet)
articles <-gsheet2tbl('https://docs.google.com/spreadsheets/d/1rAC_U5upuDPHCSLi9kieb4R5tztHVJGW84aQOf6z7Lk/edit?usp=sharing')
...
articles %>%
  ...
  labs(y = "Number of articles", x = "Year of publication", title = "Articles per year", fill = "Strain info")
```
...
articles %>%
  ...
  coord_flip() +
  labs(y = "Number of articles", x = "", title = "Journals", fill = "Strain info")
```
...
countries %>% 
  ...
  coord_flip() +
  labs(y = "Number of articles", x = "", title = "Countries with more than 2 articles")
```
...
Transitivity, also know as clustering coefficient, is the mean probability that two author with a common author are themselves co-authors. In our study the transitivity was **`r trans`**, meaning that, on average, the chance that two scholars that share a common collaborator wrote a paper together is almost 50%.
...
A different number of strains were analyzed in each of the `r nrow(articles)` reviewed articles. The total number of identified strains was `r round(sum(articles$n_isolates),1)`.   
...
The total number of indexed strains in the database was `r n_strains_yes` and non indexed was `r signif(n_strains_no)`. The percentage of indexed strains is `r round(prop_indexed,1)`.  
